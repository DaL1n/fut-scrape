...
cat data/raw_data.json | jq -c '.items[].club | {club_id: .id, club_name: .name, club_abbr_name: .abbrName}' | json2csv -p=false -d ";" -k nation_id,club_name,club_abbr_name | sort -u > data/clubs.csv
